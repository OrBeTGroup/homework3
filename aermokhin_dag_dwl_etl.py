        ...
        create or replace view aermokhin.view_payment_{{ execution_date.year }} as (
          with staging as (
            with derived_columns as (
              ...
              where cast(extact('year' from cat(pay_date as timestamp)) as int) = {{ execution_date.year }}
            ),
            hashed_columns as (
              select
                user_id,
...
for phase in ('HUBS', 'LINKS', "SATELLITES"):
    ...
    #Load HUB
        ...
        hubs = get_phase_context(phase)
        all_satelittes_loaded = DummyOperator(task_id="all_satellites_loaded", dag=dag)
